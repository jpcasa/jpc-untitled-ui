...
const Root = ({ size = 'lg', ...props }: RootProps) => {
	return (
		<RootContext.Provider value={{ size }}>
			<div
				{...props}
				className={cx('mx-auto flex w-full max-w-lg flex-col items-center justify-center', props.className)}
			/>
...
const Description = (props: ComponentPropsWithRef<'p'>) => {
	...
	return (
		<p {...props} className={cx('text-center text-sm text-tertiary', size === 'lg' && 'text-md', props.className)} />
	)
}
